...
public class PageableCategoryRepositoryImpl implements PageableCategoryRepositoryCustom {
	...
	// MIGRATION NOTE: Manual EntityManager retained in Phase 4 because the named-query paging/count behavior is not fully covered and a Spring Data rewrite could change current pagination semantics.
	@PersistenceContext
	...
	public Page<Category> listByStore(Integer storeId, Integer languageId, String name, Pageable pageable) {
	  ...
	  // MIGRATION NOTE: The legacy CATEGORY.listByStore named queries were lost during XML removal, so the original store/language/name paging semantics are restored here with equivalent JPQL.
	  String dataJpql =
