...
public class PersistableProductDefinitionMapper implements Mapper<PersistableProductDefinition, Product> {
	...
	public Product merge(PersistableProductDefinition source, Product destination, MerchantStore store,
		...
		try {
			...
			String sku = source.getIdentifier();
			if (StringUtils.isBlank(sku)) {
				sku = source.getSku();
			}
			if (StringUtils.isBlank(sku) && source.getInventory() != null) {
				// MIGRATION NOTE: Preserve legacy v1 inventory-backed SKU payloads on the v2 definition mapper after Boot 4 request binding changes.
